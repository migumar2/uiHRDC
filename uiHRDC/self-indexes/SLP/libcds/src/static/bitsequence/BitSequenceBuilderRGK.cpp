...
BitSequenceBuilderRGK::BitSequenceBuilderRGK(uint factor, uint selectsampling) {
    this->factor = factor;
    this->selectsampling = selectsampling;
}

BitSequence *BitSequenceBuilderRGK::build(uint *bitseq, size_t len) const {
    return new BitSequenceRGK(bitseq, len, factor, selectsampling);
}

BitSequence *BitSequenceBuilderRGK::build(const BitString &bs) const {
    return new BitSequenceRGK(bs, factor, selectsampling);
}
