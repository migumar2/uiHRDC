...
unsigned int* invertSA(unsigned int* sa,int len){
    ...
    int i;
    fprintf(stderr,"Trying to alloc %lu bytes\n",len*sizeof(unsigned int));
...
unsigned int* readArray(char* filename,int* alen,unsigned int size){
    ...
    fprintf(stderr,"\n il_list::utils.c::readArray()... tries to malloc %lu bytes", len * sizeof(char));
    
    if(array == NULL){
        fprintf(stderr,"ERROR: Out of memory. (utils.c::readArray())\n");
        return NULL;
    }
    size_t ret = fread(array, sizeof(char), len, file);
    fclose(file);
    if(ret!=len){
        free(array);
        return NULL;
...
unsigned int* getSA(unsigned char* text, int len, char* filename, int* salen){
    ...
    if(access(filename_sa,R_OK)!=0 || strcmp(filename,"stdin")==0 || strcmp(filename,"stdin.rev")==0){
        ...
        fprintf(stderr,"lib_il::utils.c::getSA() to malloc \n");

        sa = (unsigned int*)malloc(sizeof(unsigned int)*len);

        fprintf(stderr,"lib_il::utils.c::getSA() to call divsufsort\n");
	...
	*salen = len;
    ...
    fprintf(stderr,"lib_il::utils.c::getSA()::readArray(filename...) init\n");
    fflush(stderr);
    sa = readArray(filename_sa,salen,sizeof(unsigned int));
    fprintf(stderr,"lib_il::utils.c::getSA()::readArray(filename...) ends\n");
    fflush(stderr);
    return sa;
}
