...
int check_s9(size_t n, uint maxbits_random) {
...
//	}
	...
	printf("\n\n now compressing them with s9.");
	int N = s9_Compression(V, C, (int) n);
	printf("\n\n %zu integers were compressed within %d uints", n,N);
	
	printf("\n\n now recovering those values: s9 decoding");
	double t1 = getTime();
	s9_Decompression(C, D, (int) n);
	printf("\n decompression = %2.5f seconds", getTime()-t1); 
...
//	for (i=2;i<62;i++) {
		int err= check_s9(10000000, i);
		if (err) {
			printf("\n s9 check failed!, please check",i);
			exit(0);
		}

	 	err =check_s9(100000000, i);
		if (err) {
			printf("\n s9 check failed!, please check",i);
			exit(0);
		}
