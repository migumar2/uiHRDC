...
int build_il (uint *source, uint length, char *build_options, void **ail) {
	...
	*ail = il;
	...
	printf("\n parameters of method: B=%u, lenBitmapDiv= %d, filename = %s\n",B, lenBitmapDiv, filename);	
...
fprintf(stderr,"\n 2.0....\n");fflush(stderr);
		...
		printf("numOflists = %u, maxPost = %u, lenBitmapThreshold = %u \n", 	
		...
		fprintf(stderr,"estimated mem-usage of uncompressed posting-lists = %u bytes\n", il->sizeUncompressed);
...
fprintf(stderr,"\n 5.a....");fflush(stderr);
	...
	//			 fprintf(stderr,"\n len = %lu, B=%lu, sampleP = %lu, k=%d, numBucks= %d", n,B,sampleP, k,numBucks);fflush(stderr);	
			...
			uint *zoneOccsTmp = (uint *) malloc (sizeof(uint) * numBuckets);
...
fprintf(stderr,"\n 5.b....");fflush(stderr);
			...
			*/
			...
			uint bcssize=0;
			{
				uint nwords = il->nlists;
				uint id,j;
				uint prevvalue,val, gap, numbytes;
				uint len, sampleP,k, maxValue ;
				maxValue = il->maxPostingValue;
...
int save_il (void *ail, char *filebasename){
	...
	/** Saving lenList vector */
	{
		...
		ssize_t errio;
		errio=write(file, &(il->nlists), sizeof(uint));		
		errio=write(file, il->lenList, size);
	}

	/** Saving the postings lists using bitmaps.*/
	{
		ssize_t errio;
		errio= write(file, &(il->lenBitmapThreshold), sizeof(uint));		
		errio= write(file, &(il->bits_bitmaps), sizeof(uint));		
		errio= write(file, &(il->ints_bitmaps_aligned), sizeof(uint));	
		errio= write(file, &(il->numBitmaps), sizeof(uint));		
		//write(file, il->zoneBitmaps, (il->numBitmaps * il->ints_bitmaps_aligned) *  sizeof(uint));
		if (il->numBitmaps){	
			errio= write(file, il->zoneBitmaps, (il->numBitmaps * il->ints_bitmaps_aligned) *  sizeof(uint));
		}		
	}	
	...
	/** Saving the postings lists */
	{
		...
		errio= write(file, &(il->sizeUncompressed), sizeof(uint));		
		errio= write(file, &(il->B), sizeof(uint));		
		errio= write(file, &(il->maxPostingValue), sizeof(uint));		
		errio= write(file, &(il->zoneOccsSize), sizeof(uint)); //in # bits
		errio= write(file, &(il->numBuckets), sizeof(uint));		
		errio= write(file, &(il->bcsSize), sizeof(uint));		

		errio= write(file, il->occs, il->nlists * sizeof(uint));		
		errio= write(file, il->zoneOccs, ((il->zoneOccsSize +W-1)/W) * sizeof(uint)); //zoneOccs kbitVec[bucketSize]
		errio= write(file, il->bcs, il->bcsSize * sizeof(byte));					
		errio= write(file, il->bcsptr, (il->nlists +1)* sizeof(uint));				
	}
	
	fprintf(stderr,"\n ########## il->sizeUncompressed = %d ",il->sizeUncompressed   );
	fprintf(stderr,"\n ########## il->B = %d ",il->B   );
	fprintf(stderr,"\n ########## il->maxPostingValue = %d ",il->maxPostingValue   );
	fprintf(stderr,"\n ########## il->zoneOccsSize = %u bits = %ld bytes",il->zoneOccsSize, ((il->zoneOccsSize + W -1)/W * sizeof(uint)) );
	fprintf(stderr,"\n ########## il->numBuckets = %d ", il->numBuckets  );
	fprintf(stderr,"\n ########## il->bcsSize = %d ", il->bcsSize  );
	fprintf(stderr,"\n ##########     zoneBitmapsSize = %ld bytes", (il->numBitmaps * il->ints_bitmaps_aligned) *  sizeof(uint) );
	fprintf(stderr,"\n ########## il->numBitmaps = %u, bytesPerbitmap = %ld ", il->numBitmaps, (il->ints_bitmaps_aligned)* sizeof(uint)   );
...
int load_il (char *filebasename, void **ail) {
	...
	{	
		errio = read(file, &(il->nlists), sizeof(uint)); // number of posting lists 
	
		il->lenList = (uint *) malloc ( (il->nlists) * sizeof(uint));  // the lenList array
		errio = read(file, il->lenList, ((il->nlists) * sizeof(uint)) );
	}

	/** Loading the postings lists using bitmaps.*/
	{
		errio = read(file, &(il->lenBitmapThreshold), sizeof(uint));		
		errio = read(file, &(il->bits_bitmaps), sizeof(uint));		
		errio = read(file, &(il->ints_bitmaps_aligned), sizeof(uint));	
		
		errio = read(file, &(il->numBitmaps), sizeof(uint));		
		...
		if (il->numBitmaps){
			il->zoneBitmaps = (uint *) malloc (sizeof(uint) * ( il->numBitmaps * il->ints_bitmaps_aligned));
			il->zoneBitmaps[( il->numBitmaps * il->ints_bitmaps_aligned) -1] =0000; //so valgrind is shut up		
			errio = read(file, il->zoneBitmaps, ( il->numBitmaps * il->ints_bitmaps_aligned) *  sizeof(uint));
		}
	...
	{		
		//some integers
		errio = read(file, &(il->sizeUncompressed), sizeof(uint));					
		errio = read(file, &(il->B), sizeof(uint));	
		errio = read(file, &(il->maxPostingValue), sizeof(uint));	
		errio = read(file, &(il->zoneOccsSize), sizeof(uint)); //in # bits
		errio = read(file, &(il->numBuckets), sizeof(uint));		
		errio = read(file, &(il->bcsSize), sizeof(uint));		
		
		//vector occs.
		il->occs = (uint *) malloc (il->nlists * sizeof(uint) );		
		errio = read(file, il->occs, il->nlists * sizeof(uint)); //wcsa->nwords was loading in "loadVocabulary"
		...
		il->zoneOccs[((il->zoneOccsSize +W-1)/W)-1] =0000; //so valgrind is shut up		
		errio = read(file, il->zoneOccs, ((il->zoneOccsSize +W-1)/W) * sizeof(uint)); 

		//vector bcs.
		il->bcs = (byte *) malloc (il->bcsSize * sizeof(byte));		
		errio = read(file, il->bcs, il->bcsSize * sizeof(byte));		
		
		//vectir bcsptr.
		il->bcsptr = (uint *) malloc (sizeof(uint) * (il->nlists +1));
		errio = read(file, il->bcsptr, (il->nlists +1)* sizeof(uint));		
...
int extractList_il (void *ail, uint id, uint **list, uint *len) {
	...
	register uint maxValue = il->maxPostingValue;
	...
	register uint e,s,bcpos,end;
	uint pos, gap, prevvalue;
	uint ne,ns;
	...
	ns=0; e=0;
	uint i=0; //position inside alist[]
...
int lookup2(void *ail, uint id1, uint id2, uint *noccs, uint **occs){
	...
	{	
		...
		if (shouldUseBitmap(il,second)) {
			...
			for (j=0; j<n;j++) {
				result = (*defaultFsearch) (ail, second, alist[j]); //call to fsearch;
				if (result) {
...
*/
	...
	if (numbytecodes == 0) { //intersecting only bitmaps --> mergeNbitmap
		...
		extractList_il (ail, idss[0].value, &alist, noccs); //uncompressing the shortest list (used in a svs approach).
		n= *noccs;
		matches =n;
