 ...
 * 
 * search_il.h: test search program following the interface in "interface.h".
 *   
...
static char matlab[100]="moffat";
static char outfilename[100]="moffat_";
...
void readPatternSetsIntersect (uint *numIdsPerPattern, uint *numOfPatterns, uint ***patternSets) {
	int errio;
	errio = fscanf(stdin,"##numIdsPerPattern: %9d\n",numIdsPerPattern);
	errio = fscanf(stdin,"##numOfPatterns : %9d\n",numOfPatterns);
...
/**/	fprintf(timeFile,"\n Searching!!: %s %s %s %s %s",argv[0],argv[1],argv[2],argv[3], params);
	...
	buildBell_il(Index, "bytecodesBitmapBellMoff.m");
	 
	Load_time = getTime () - Load_time;
	Load_time *= TIMEFACTOR_TO_SEC;
	fprintf (stderr, "\nLoad index time = %.6f secs\n", Load_time);
