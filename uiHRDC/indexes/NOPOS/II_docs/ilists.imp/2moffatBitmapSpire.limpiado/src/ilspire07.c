 ...
 * 
 * ilspire07.c: Implementation of the functions in the interfaz in "interface.h".
 *   
...
static int is_delimeter(char *delimiters, char c) {
   int i=0,len_delimiters=strlen(delimiters);
   int is=0;
   for (i=0;i<len_delimiters;i++)
     if (c == delimiters[i]) is=1;
   return is;
}
...
int build_il (uint *source, uint length, char *build_options, void **ail) {
	...
	*ail = il;
	...
	printf("\n parameters of method: sampleK=%u, lenBitmapDiv= %d, filename = %s\n",sampleK, lenBitmapDiv, filename);
...
int save_il (void *ail, char *filebasename){
	...
	{	
		...
		errio= write(file, &(il->sampleK), sizeof(uint));		
		errio= write(file, &(il->lenBitmapThreshold), sizeof(uint));		
		errio= write(file, &(il->zoneOccsSize), sizeof(uint)); //in # bits
		errio= write(file, &(il->numBuckets), sizeof(uint));		
		errio= write(file, &(il->absOffsetSize), sizeof(uint));		
		errio= write(file, &(il->bcOffsetSize), sizeof(uint));		
...
int load_il (char *filebasename, void **ail) {
	...
	{		
		...
		errio =read(file, &(il->sampleK), sizeof(uint));
		errio =read(file, &(il->lenBitmapThreshold), sizeof(uint));				
		errio =read(file, &(il->zoneOccsSize), sizeof(uint)); //in # bits
		errio =read(file, &(il->numBuckets), sizeof(uint));		
		errio =read(file, &(il->absOffsetSize), sizeof(uint));		
		errio =read(file, &(il->bcOffsetSize), sizeof(uint));	
		...
		errio =read(file, il->zoneOccs, ((il->zoneOccsSize +W-1)/W) * sizeof(uint)); //zoneOccs kbitVec[bucketSize]
...
uint  sizetobits(void *ail) {
	...
	// size of lenList
	maxs=0;
	for(uint i=0;i<il->nlists;i++)       //** calculates el max symbol in lenlist[];
	...
	// size of occs
	maxs=0;
	for(uint i=0;i<il->nlists;i++)       //** calculates el max symbol in occs[];
...
//	*size += (il->nlists + 1)* sizeof(uint);  //il->bcsptr 
	...
	*size += (il->zoneOccsSize + W -1)/W * sizeof(uint);
	...
	fprintf(stderr,"\n size of zoneOccs = %lu bytes \n",(ulong)((il->zoneOccsSize + W -1)/W * sizeof(uint)));

	*size += il->bcsSize ;

	fprintf(stderr,"\n size of bitmaps = %lu bytes \n",(ulong)(( il->numBitmaps * il->ints_bitmaps_aligned) * sizeof(uint) ));
