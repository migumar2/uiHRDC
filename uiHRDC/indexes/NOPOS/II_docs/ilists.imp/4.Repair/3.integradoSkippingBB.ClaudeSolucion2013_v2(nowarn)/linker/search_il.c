...
static char matlab[100]="repairSkip";
static char outfilename[100]="repair_skip";
...
void endResultsRemoveZeroes  (char *matlab, char* outfile, uint numIters) {
	FILE *f = fopen(outfile,"a");
	uint i,j,x;
	double y,sum;
	uint diffs =1;
...
#ifdef DO_LOG
////** occs[0] guarda el número de "elementos" que han sido extraidos... (es decir el primero es occs[1])
////** por ello hay que pasar (occs+1);
//out_results(&(IDs[currID]), 1, (occs+1), numocc);
...
do_extract_no_malloc(uint numIdsToExtract, uint REPETITIONS)
{
	...
	if (numIdsToExtract > numOfIDs) {
		fprintf(stderr,"\nBuild_il: Error during extract %lu/%lu IDs\n", (ulong)numIdsToExtract, (ulong)numOfIDs);
		exit(0);
	}
...
/**/	fprintf(timeFile,"\n\t occs = %9lu :: time = %.8f msecs ",(ulong)tot_numocc,tot_time*1000);
...
do_intersect2(uint numPhrases, uint REPETITIONS)
{
	...
	if (numPhrases > numOfPatterns) {
		fprintf(stderr,"\nBuild_il: Error during intersect2 %lu/%lu IDs\n", (ulong)numPhrases, (ulong)numOfPatterns);
		exit(0);
	}
...
/*
double
getTime (void)   // OJO!!! EN MICROSEGUNDOS CORREXIDO SIGIR!!!
{
	double usertime, systime;
	struct rusage usage;
	...
	usertime = (double) usage.ru_utime.tv_sec *10000000.0 +
		(double) usage.ru_utime.tv_usec * 1.0 ;

	systime = (double) usage.ru_stime.tv_sec*1000000.0 +
		(double) usage.ru_stime.tv_usec  * 1.0;

	//return (usertime + systime);
	//return (usertime/1000 );
	return (usertime);   // RETURNED IN SECONDS * 1000000  --> MICROSECONDS
}
*/
