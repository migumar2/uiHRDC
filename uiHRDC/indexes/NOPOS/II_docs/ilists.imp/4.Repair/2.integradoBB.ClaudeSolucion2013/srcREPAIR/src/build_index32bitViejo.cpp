...
int main(int argc, char ** argv) {	  
  ...
  fread(&edges,sizeof(uint),1,fp);          //** reads the number of edges
  printf("\n\n\n *****ejes = %u\n ",edges);
fflush(stdout);

  int * text = (int*) calloc(mem,sizeof(uint));
...
///** FARI */
  ...
  uint plen = bits(edges);    //** ptr a los lugares donde están los ejes para cada nodo. en el texto T, no en el comprimido (por ahora) **/
  ...
  RePair * rc = new RePair((ulong*)text,(ulong)(edges+nodes),(ulong)mem,fname,(ulong)K);
  rc->compress(nodes,CORTE_REPAIR);   //** nodes es el número de nodos, y también indica que los ejes toman "valores > nodes"
  rc->save(nodes);
  ...
  fwrite(&edges,sizeof(uint),1,f_out);
  fwrite(offsets,sizeof(uint),offsets_len,f_out);
  fclose(f_out);
