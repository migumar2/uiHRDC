...
/*
size_t MyReadBuff(void *buff, void *data, size_t size)
  { if (memcpy(data,buff,size)) return size; 
  	return 0;}

int MyReadBuffAndCheck(boid *buff, void *data, size_t size)
  { return (MyReadBuff(buff, data, size) == size); }
*/
...
int main3(FILE *inFile, FILE *outFile, char *rs)
{
  ...
  /* Read LZMA properties for compressed stream */
	...
	fprintf(stderr,"\n FARI:: sizeof(properties) = %d bytes", sizeof(properties));
  ...
  fprintf(stderr,"\n lzma properties: size = %d, LZMA_PROPERTIES_SIZE);
