...
int compressDataExt(uchar *data, uint n, char *path2LzmaEncoder) {
	...
	unlink(TMP_FILE_TO_COMPRESS_VBYTE);
	if (! (f = fopen(TMP_FILE_TO_COMPRESS_VBYTE, "wb+"))) {
	...
	sprintf(command,"%s e %s %s",path2LzmaEncoder, TMP_FILE_TO_COMPRESS_VBYTE, TMP_FILE_COMPRESSED_LZMA);
...
int loadDataExt(uchar **data, uint *readen) {
	...
	uchar *buffer;
	unlink(TMP_FILE_TO_COMPRESS_VBYTE);
	if (! (f = fopen(TMP_FILE_COMPRESSED_LZMA, "rb"))){ 
		fprintf(stderr, "Can ot open temp files for compression. Exitting...!\n"); exit(0);
	}	

	lenFile = getfileSize(TMP_FILE_COMPRESSED_LZMA);
...
int createLzmaExt(uchar *inb, uint inn, void *lzmadataV) {
	...
	{ /** Read uncompressed size (inb[5..8] && skipping inb[9..12] )*/
		...
		inStream = &(inb[ (sizeof(lzmadata->properties)) ]); /** position &inb[5] */
		for (i = 0; i < 4; i++) { 
			b= *inStream++;
...
uint writeBuffLzmaStruct(uchar *outstream, uint pos, void *lzmadataV) {
	...
	dst = &(outstream[pos]);
	memcpy ((void *) dst , (const void *) lzmadata->properties, sizeof(lzmadata->properties) );
