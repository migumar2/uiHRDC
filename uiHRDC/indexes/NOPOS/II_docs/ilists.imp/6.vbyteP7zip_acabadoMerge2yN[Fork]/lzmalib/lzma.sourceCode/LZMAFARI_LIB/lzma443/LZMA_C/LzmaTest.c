...
int main3(FILE *inFile, FILE *outFile, char *rs)
{
  ...
  #ifndef _LZMA_IN_CB
  {
    ...
    compressedSize = (SizeT)(length - (LZMA_PROPERTIES_SIZE + 8));  /** fari <<-- !! length -13 ! */
  }
  #endif
...

  {
    ...
    for (i = 0; i < 8; i++)
    {
      ...
      if (!MyReadFileAndCheck(inFile, &b, 1))
        return PrintError(rs, kCantReadMessage);
	fprintf(stderr,"\n \t\tFARI:: leído==> \"b\" = %d (si <>255 ==> waitEOS =false)",  b);
    ...
    /* notar que waitEOS será siempre falso, salvo que size> [FF]FFFFFF,FFFFFFFF 
       es decir, waitEOS = false si "conocemos la longitud del texto comprimido" !!
     */
    
   fprintf(stderr,"\n\tFARI::outSize = %d bytes (outSizeHigh = %d)", outSize, outSizeHigh);
   fprintf(stderr,"\n\tFARI::wait for EOS (end of stream) marker = %d (1=true, 0=false)\n", waitEOS);
