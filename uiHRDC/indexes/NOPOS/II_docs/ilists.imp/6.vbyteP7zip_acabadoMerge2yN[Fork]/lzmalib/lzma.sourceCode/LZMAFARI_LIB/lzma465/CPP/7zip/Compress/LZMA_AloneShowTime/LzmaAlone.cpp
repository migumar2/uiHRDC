...
int main2(int n, const char *args[])
{
  ...
  if (encodeMode)
  {
	...
	/** FARI :: writes 5 uints (basically the size of the dictionary) */
    encoderSpec->WriteCoderProperties(outStream);
	/** end FARI :: writes 5 uints (basically the size of the dictionary) */
	...
	/** FARI :: writes the size of the source file in bytes, (uses 8 bytes= 2*uint32) */
    for (int i = 0; i < 8; i++)
    {
    ...
    /** end FARI :: writes the size of the source file in bytes */
...
{
  try { 
		double initime = iniTimeFari();
	  
		int errval = main2(n, args); 
	  
		double totaltime = endTimeFari(&initime);
				
		saveTimesFile(totaltime);
