...
*/
    ...
    auto tickt = get_time_usecs();
	uint64_t tot_res=0;
    for (auto const& query: queries) {
		uint64_t result = query_op(index, query);   //first run is not timed
		do_not_optimize_away(result);
		tot_res +=result;
        ...
        double elapsed = double(get_time_usecs() - tick)/runs;
        query_times.push_back(elapsed);
    }
    double tott = double(get_time_usecs() - tickt)/runs;
    ...
    if (false) {
        ...
        double avgocc = std::accumulate(query_times.begin(), query_times.end(), double()) / tot_res;
        double avgpat = std::accumulate(query_times.begin(), query_times.end(), double()) / query_times.size();
        ...
        logger() << "Tot time (micros): " << tott << std::endl;
        logger() << "Time per occ (micros): " << avgocc << " occs= " << tot_res << std::endl;
        logger() << "Mean (micros per pat): " << avgpat << std::endl;
        logger() << "Mean (micros per occ): " << avgocc << std::endl;
        ...
        stats_line()
            ...
            ("tottime (micros)", tott)
            ("noccs", tot_res)
            ("avg (micros per pat)", avgpat)
            ("avg (micros per occ)", avgocc)
            ("q50", q50)
            ("q90", q90)
            ("q95", q95)
            ;
