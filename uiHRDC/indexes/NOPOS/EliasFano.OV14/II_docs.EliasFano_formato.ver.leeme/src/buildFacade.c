...
//		for (i = 0; i<100; i++) {
	...
	{	
		...
		uint **occList; uint **tfd; uint *idsindoc; uint *lenList;
		
		t1 = getSYSTimeBF();
		fprintf(stderr,"\n... Entering createListsOfOccurrences (2nd pass) \n"); fflush(stderr);	
		
		createListsOfOccurrences (*index, &occList,&tfd, &idsindoc, &lenList, docboundaries, ndocs);
...

		{
			...
			for (i=0;i<ndocs;i++) {
				fwrite(&(idsindoc[i]),sizeof(uint),1,f);
			}
			fclose(f);
		}	
