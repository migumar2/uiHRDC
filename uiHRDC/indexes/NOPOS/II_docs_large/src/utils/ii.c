...
int CreateVocabularyOfWords (void *index, ulong *docboundaries, uint ndocs){
	...
	{ 
		...
		for (j=0;j<ndocs;j++) {
			if (!(j%progress_cut)){
				printf("# 1st pass %2.2f%% docs processed\r", 100.0 * ((j*1.0)/(ndocs)) );
				fflush(stdout);
			}		
	...
	printf("# 1st pass %2.2f%% docs processed\n", 100.0 );
...
int createListsOfOccurrences(void *index, uint ***occList, uint **lenList, ulong *docboundaries, uint ndocs) {
	...
	for (i=0;i<ndocs;i++) {
		...
		if (!(i%progress_cut)){
			printf("#2nd pass # %2.2f%% docs processed\r", 100.0 * ((i*1.0)/(ndocs)) );
			fflush(stdout);
		}
	...
	printf("#2nd pass # %2.2f%% docs processed\n", 100.0 );
