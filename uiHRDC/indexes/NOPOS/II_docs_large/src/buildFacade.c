...
int load_index(char *filename, void **index){
	...
	error = load_il((char*) filename,&(wcsa->ils));
 	...
 	IFERRORIL(error);
	fprintf(stderr,"\n \t*loaded compressed inverted lists structure: %lu bytes\n", (ulong)sizeil);
...
int index_info(void *index, char msg[]) {
	...
	sprintf (msgt,"\t Inverted list-structure = %lu bytes.\n",sizeil );	strcat(msg,msgt);
...
//		for (i = 0; i<100; i++) {
	...
	wcsa->text = NULL;
		...
		sourcelen_il = (ulong)source_il_ulong;
...
*/	
	...
	/** 4 ** Loading the uncompressed posting lists previously created by the indexer. */
		...
		fprintf(stderr,"\n MAXPOST loaded = %u, source_il_len = %lu \n\n",maxPost,(ulong)sourcelen_il);
		fprintf(stderr,"\n NLISTS loaded = %u, MAXPOSTS_sET %u \n\n",source_il[0],source_il[1]);
