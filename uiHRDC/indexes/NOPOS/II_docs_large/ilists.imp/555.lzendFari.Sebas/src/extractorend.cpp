...
ExtractorEnd::ExtractorEnd(char* filename, unsigned int sampling):Extractor(filename, sampling){
    ...
    for(unsigned int i=0;i<this->len;i++){
        if(this->plen[i]==1){
			...
			unsigned int sourcesph = get_field_LL(this->sources, this->sourcesbits, i); //unsigned int sourcesph = this->sources[i];
            unsigned int r = this->dc->rank(sourcesph+this->plen[i]-2)-1;
            this->sources_id[i] = r;
        }
    }  
}
