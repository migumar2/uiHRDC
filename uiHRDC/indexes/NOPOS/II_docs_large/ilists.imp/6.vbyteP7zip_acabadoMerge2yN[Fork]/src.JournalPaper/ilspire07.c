...
int compressedParallelChild(t_il* il, tvbytelzma *V, uint nwords, uint minbcssize, uint IDP) {
...
int compressedParallelFather(t_il* il, tvbytelzma *data, uint nwords, uint minbcssize) {
	...
	for (i=0; i< NPROC;i++) {
		...
		if (pid ==0) { //hijo
			sleep(5);
			compressedParallelChild(il,data,nwords,minbcssize,i);		
			exit(0);
		}
		else if (pid >0){
			PIDS[i]= pid;
