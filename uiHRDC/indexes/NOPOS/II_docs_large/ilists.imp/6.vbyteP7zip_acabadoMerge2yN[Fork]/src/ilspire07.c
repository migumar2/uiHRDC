...
size_t fileSizeLzma (char *filename){
	FILE *fpText;
	size_t fsize;
...
tvbytelzma *generateData(t_il *il, uint nwords) {
	tvbytelzma *V = (tvbytelzma*) malloc (nwords * sizeof(tvbytelzma));
	...
	for (id=0; id<nwords; id++) {
		...
		V[id].inputdata = bcsdata;
		V[id].ilen = lenbcs;
		V[id].outputdata = NULL;
		V[id].olen =0;
	}
	return V;	
}
...
int compressedParallelChild(t_il* il, tvbytelzma *V, uint nwords, uint minbcssize, uint IDP, uint nprocs) {
	...
	for (id=IDP; id<nwords; id+=nprocs) {
		byte *inb=NULL; uint inn;  //pointer to compressed file in RAM and its size.
		lenbcs = V[id].ilen;	
		byte *bcsdata = V[id].inputdata;
		...
		{ 
			...
			if (!res) {
				...
				fprintf(stdout,"\n [@@%lu@@] \n[id= %6d/%6d] lenbcs = %8d, lenbcs+lzma = %8d ==> usesLzma = %d (f/t?) (bcpos[%lu]=%d)",(ulong)getpid(),id, nwords,lenbcs,lenlzma, (lenlzma<lenbcs),(ulong) getpid(), bcpos);
				if (lenlzma < lenbcs) {
					V[id].outputdata = (byte *) malloc(lenlzma * sizeof(byte));
					
					writeBuffLzmaStruct(V[id].outputdata,0,il->lzmadata);
					V[id].olen = lenlzma;
					
					bcpos +=lenlzma;
				}
				else {
					V[id].outputdata = (byte *) malloc(lenbcs * sizeof(byte));
					
					memcpy( (void *) (V[id].outputdata), (const void *) bcsdata, lenbcs);
					V[id].olen = lenbcs;
					bcpos +=lenbcs;
				}
	...
	//now outpus data to pids! file
		char fname [2048];
		sprintf(fname,"vbyte.lzma.%lu",(ulong) getpid());
		FILE *f = fopen(fname,"w");
		if (f) {
			printf("\n child %u opens file %s to output its data\n",IDP, fname);
			fflush(stdout);
			fflush(stderr);
		}
		else {
			printf("\n child %u cannot open file %s to output its data\n",IDP, fname);
			fflush(stdout);
			fflush(stderr);
			exit(0);
		}
		...
		byte *b;
		uint i = id;
		for (; id < nwords; id+=nprocs) {
			len = V[id].olen;
			b =V[id].outputdata;
			fwrite(&len,sizeof(uint),1,f);
			fwrite(b,sizeof(byte),len,f);
			free(V[id].outputdata);
		}			
		fclose(f);
		printf("\n data for CHILD %lu has been saved to disk\n",(ulong) i);
		free(V);

		for (i=0;i<il->nlists;i++) free(OCCLISTCHILD[i]);
		free(OCCLISTCHILD);


		if (il->lenList) free(il->lenList);
		if (il->occs) free(il->occs);
...
int compressedParallelFather(t_il* il, tvbytelzma *data, uint nwords, uint minbcssize) {
	uint nprocs = getNprocsToUse();
	//pid_t PIDS[NPROC];
	...
	for (i=0; i< nprocs;i++) {
		...
		if (pid ==0) { //hijo
			sleep(5);
			compressedParallelChild(il,data,nwords,minbcssize,i,nprocs);		
			exit(0);
		}
		else if (pid >0){
			PIDS[i]= pid;
...
*/	
	...
	for (i=0;i< nprocs; i++) {
		printf("\n ********* PADRE ESPERANDO POR %lu HIJOS !!! \n",(ulong)nprocs-i);
		int statusX;
		pid = wait(&statusX);
		printf("\n el hijo %6lu ha terminado \n",(ulong) pid);
		
//		pid=PIDS[i];
		
		char fname [2048];
		sprintf(fname,"vbyte.lzma.%lu",(ulong) pid);
		FILE *f = fopen(fname,"r");
		
		uint id;
		size_t flen= fileSizeLzma(fname);
		byte * buffer = (byte *) malloc (sizeof(char) * flen);
		fread(buffer, sizeof(byte), flen, f);
		fclose (f);
		id = 0;
		while (PIDS[id] != pid) {
			id++;
		...
		for (; id < nwords; id+=nprocs) {
			...
			b+= sizeof(uint);
			data[id].outputdata = (byte *) malloc (sizeof(byte) * (*len));
			memcpy(data[id].outputdata, b, *len);
			b+= *len;		
		}			
		printf("\n data for CHILD %lu has been recovered\n",(ulong) i);
		free(buffer);
	}
...
// ahora carga todo de los ficheros de los hijos
}
...
int build_il (uint *source, ulong length, char *build_options, void **ail) {
	...
	*/
	...
	fprintf(stderr,"\n paso 2 ");fflush(stderr);		
		...
		il->occs = (ulong *) malloc (sizeof(ulong) * (il->nlists+1));
			...
			*/
			ulong bcssize=0;
			{
				...
				fprintf(stderr,"\n Number of gaps to store is = %lu",counterBCcodes);	
				fprintf(stderr,"\n Size of bcs Vector should be  %lu bytes",bcssize);
			}
			
		il->bcsSize = bcssize;
		il->bcs = (byte *) malloc (sizeof(byte) * il->bcsSize);		
		if (!il->bcs) printf("\n malloc failed for il->bcs: %lu bytes\n",sizeof(byte) * il->bcsSize);
		...
		/** 5.c ** encodes gaps into bcs  */
		{
			...
			fprintf(stderr,"\n Gaps were encoded with bc ==> %lu bytes",bcpos);
		}		
...
//			uint *newoccs = (uint *) malloc (sizeof (uint) * (nwords+1));
			...
			initLzmaStruct (&il->lzmadata);
			byte *newbcs = (byte *) malloc (sizeof(byte) * il->bcsSize);
			ulong *newoccs = (ulong *) malloc (sizeof (ulong) * (nwords+1));
			il->useslzma = (uint *) malloc (sizeof(uint) * ((nwords + W -1)/W));
			for (j=0;j< ((nwords + W -1)/W) ; j++) il->useslzma[j]= 0000;
			...
			for (id=0; id<nwords; id++) {
				...
				{ 
					...
					printf("\n [id= %6d/%6d] lenbcs = %lu, lenbcs+lzma = %lu ==> usesLzma = %d (f/t?) (bcpos=%lu)",id, nwords,lenbcs,lenlzma, (lenlzma<lenbcs),bcpos);
					if (lenlzma < lenbcs) {
...
int save_il (void *ail, char *filebasename){
	...
	/** Saving the postings lists using bytecodes or bytecodes+lzma */
	{
		...
		write(file, &(il->bcsSize), sizeof(ulong));		

		write(file, il->occs, (il->nlists +1) * sizeof(ulong));		
...
int load_il (char *filebasename, void **ail) {
	...
	{		
		...
		read(file, &(il->bcsSize), sizeof(ulong));		
		
		//vector occs.
		il->occs = (ulong *) malloc ((il->nlists+1) * sizeof(ulong) );		
		read(file, il->occs, (il->nlists+1) * sizeof(ulong)); //wcsa->nwords was loading in "loadVocabulary"
...
//	fprintf(stderr,"\n\t size of byteCodes = %d bytes",il->bcsSize);
	...
	fprintf(stderr,"\n\t size of: lenlist+occs = %lu bytes; byteCodes = %lu bytes;", (ulong)sizetobits(ail),il->bcsSize);
...
int merge2 (void *ail, uint id1, uint id2, uint *noccs, uint **occs){
	...
	t_il *il=(t_il *) ail;	
	//fprintf(stderr,"\n call to merge2lzma, id1=%u, id2=%u", id1,id2);
...
/* intersects to streams of byte-codes belonging to term i1, and i2 
 ...
 * merges V with the lists of occurrences of the lists "id2".
...
int merge2_UINT_BCS (uint *V, uint n1, uint id2, uint n2, byte* src2, uint *noccs){
	...
	bcpos1=bcpos2=0;	
	val1 = V[bcpos1]+DOCid_ADD; bcpos1++;
...
int mergeN (void *ail, uint *ids, uint nids, uint *noccs, uint **occs){
	...
	{  //bytecodes (& could also be bytecodes+lzma) --> extract 1st + svsN.
		...
		for (i=1; i<numbytecodes; i++){ // the "nids-1" remaining posting-lists.
			...
			merge2_UINT_BCS(firstlist,n,id2,n2,src2, &matches);
