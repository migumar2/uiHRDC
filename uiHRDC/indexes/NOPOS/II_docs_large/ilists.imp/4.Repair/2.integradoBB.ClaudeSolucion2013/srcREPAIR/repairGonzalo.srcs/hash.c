...
typedef unsigned long  relong;
#define LPRIME ((relong)767865341467865341L)
#define PRIME 2013686449L

long searchHash (Thash H, Tpair p) // returns id 

  { relong u = ((relong)p.left)<<(8*sizeof(int)) | (relong)p.right;
    long k = ((LPRIME*u) >> (8*sizeof(int))) & H.maxpos;
...
void deleteHash (Thash *H, long id) // deletes H->Rec[id].pair from hash
...
Thash createHash (long maxpos, Trarray *Rec)
  ...
  { Thash H;
    ...
    H.maxpos = maxpos;
    H.used = 0;
    H.table = (void*)malloc((1+maxpos)*sizeof(long));
    for (i=0;i<=maxpos;i++) H.table[i] = -1;
    H.Rec = Rec;
    return H;
  }
  
static long finsertHash (Thash H, Tpair p) 
...
void insertHash (Thash *H, long id) // inserts H->Rec[id].pair in hash 
...
void hashRepos (Thash *H, long id)
