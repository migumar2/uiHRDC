...
int read_file(char *filename, uint **sourceInts, ulong *length) {
  ...
  *length = ftell(infile)/(sizeof(uint));
  ...
  fflush(stdout);
  cout << " [build_bitmap] I have read " << (*length)*sizeof(uint) << " bytes from postings.post file" ;
  cout << " [build_bitmap] I have read " << (count)*sizeof(uint) << " bytes from postings.post file" ;
...
printf("\n\n opening file %s",filename);
  ...
  fread(sourcelen,sizeof(ulong),1,infile);
