...
long insertArray (Tarray *A, long pair)

   { if (A->size == A->maxsize)
	{ if (A->maxsize == 0)
	     { A->maxsize = A->minsize;
	       A->pairs = malloc (A->maxsize * sizeof(long));
	     }
	  else
	     { A->maxsize /= A->factor;
	       A->pairs = realloc (A->pairs, A->maxsize * sizeof(long));
	     }
	}
     A->pairs[A->size] = pair;
     return A->size++;
...
Tarray createArray (float factor, long minsize)
