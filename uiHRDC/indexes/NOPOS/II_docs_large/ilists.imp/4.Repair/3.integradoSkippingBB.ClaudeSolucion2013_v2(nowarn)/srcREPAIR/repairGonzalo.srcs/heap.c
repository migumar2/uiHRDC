...
Theap createHeap (long u, Trarray *Rec, float factor, long minsize) 
  ...
  { Theap H;
    long i;
    H.sqrtu = 2;
    while ((long)H.sqrtu * H.sqrtu < u) H.sqrtu++;
...
static void move (Tarray A, long i, long j, Trecord *rec)
...
void incFreq (Theap *H, long id) // inc freq of pair Rec[id]

  { Trecord *rec = H->Rec->records;
    long freq = rec[id].freq++;
    long hpos = rec[id].hpos;
    Thnode *p;
    Thfreq *f,*lf;
    long fp,lfp;
if (PRNH) prnH(H);
...
void decFreq (Theap *H, long id) // dec freq of pair Rec[id]
...
void insertHeap (Theap *H, long id)  // with freq 1
...
void heapRepos (Theap *H, long id) // repositions pair
