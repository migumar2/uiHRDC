...
size_t *offset_src;    //para cada símbolo en C[0..m], indica cual era el offset en la secuencia original [1..n]
//uint src_pos;
...
void compress_pair_table() {
  ...
  symbols_aux[((m/32*nbits + m%32*nbits/32)+1)-1] = 0000;  //avoids valgrind warning
...
//@@      distancia+=BR->select(i+2)-BR->select(i+1);
      distancia+=offset_src[i+1]-offset_src[i];//@@  
...
int main(int argc, char ** argv) {
	...
	size_t rf;
	rf= fread(&min_value,sizeof(uint),1,grfp);
	rf= fread(&max_value,sizeof(uint),1,grfp);
	rf= fread(&max_assigned,sizeof(uint),1,grfp);
	rf= fread(&m,sizeof(uint),1,grfp);
	rf= fread(&n,sizeof(ulong),1,grfp);
	uint nodes;
  rf= fread(&nodes,sizeof(uint),1,grfp);
	printf("m=%u\n",++m);
	symbols = new uint[m];
	
	
  assert(symbols!=NULL);
	printf("read=%ld\n",(long)fread(symbols,sizeof(uint),m,grfp));
	fclose(grfp);
  ...
  assert(symbols_pair!=NULL);
	...
	offset_src = new size_t[m+1];
	compress_pair_table();  //** !!!  Crea symbols_new, symbols_new_len   y rellena BR
	...
	rf= fwrite(&symbols_new_len,sizeof(uint),1,cdictfp);
	rf= fwrite(symbols_new,sizeof(uint),symbols_new_len,cdictfp);
	printf("save BRR = %u\n",BRR->save(cdictfp));
	fclose(cdictfp);
	...
	rf=fwrite(&min_value,sizeof(uint),1,cpsifp);
	rf=fwrite(&max_value,sizeof(uint),1,cpsifp);
	rf=fwrite(&max_assigned,sizeof(uint),1,cpsifp);
	rf=fwrite(&nbits, sizeof(uint),1,cpsifp);
	rf=fwrite(&m,sizeof(uint),1,cpsifp);
	rf=fwrite(&n,sizeof(ulong),1,cpsifp);
  uint wr = 0;
  ...
  //printf("Writing: %u (m=%u nbits=%u) => %u\n",(m/32*nbits+m%32*nbits/32+1),m,nbits,wr);
	...
	//printf("save BR = %u\n",BR->save(cpsifp));
	//
	fwrite(offset_src,sizeof(size_t),m+1, cpsifp);
	
	fclose(cpsifp);

  cout << "\n ================== compress_dictionary program ends ==================" << endl << endl;
	return 0;
}
