...
/*------------------------------------------------------------------
 ...
 ------------------------------------------------------------------*/
BufferRead bufCreate (unsigned long minDataInBuffer)
{
...
void bufSetFile (BufferRead B, FILE *file)
{
	...
	B->dsize = fread(B->data, sizeof(char), B->size, B->file);
}
  
/*------------------------------------------------------------------
 Gets buffer size of B 
------------------------------------------------------------------*/
long bufSize (BufferRead B)
{
	return B->size;
}
...
static ulong count=0;
	...
	register long mov = B->dsize - (src - dst);
	...
	B->dsize = fread(dst, sizeof(char), mov, B->file);
	B->fpos += mov;
	count +=mov;
	printf("\n reading file at position %lu",count);
