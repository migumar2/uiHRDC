...
void parseFile (char* infile,  uint inFileSize, char *nameoutFile, uint numberOfWords_min, uint numberOfWords_max, uint numberOfPhrases) {
	...
	uint *cutPos;  //positions where a different word starts from the random phrase.
	uint *sizes;
	uint cutPosTot;
	...
	while ( iterations < numberOfPhrases) {
		if (!(iterations %1000)) fprintf(stderr,"#");
	...
	*/
		...
		//	if (len <5) continue;
			...
			//			{uint kk=0;
			//			fprintf(stderr,"\nPOSITIONS: ");
			//			   for (kk=0;kk<cutPosTot;kk++) {fprintf(stderr,"[%2u]",cutPos[kk]);}
			//			fprintf(stderr,"\n SIZES   : ");
			//			   for (kk=0;kk<cutPosTot;kk++) {fprintf(stderr,"[%2u]",sizes[kk]);}
			//			}
	...
	fprintf(stderr, "... EXTRACTION OF WORDS FINISHED \n");
...
void showCreatedPhrases(FILE *infile) {
	...
	fprintf(stderr," numOfFrase = %9d and numberOfWordsPerPhrase = %9d\n",numPhrase, numOfWords);
		
	for (i=0;i<numPhrase;i++) {
		patt =inputWord(infile, &len);	
		fprintf(stderr,"[%d]:::  patLen= %3d, patt=<<%s>>\n",i,len,patt);
		free(patt);
	}
}
...
unsigned long fileSize (char *filename) {
	...
	if (fpText) {
		...
		fsize= ftell(fpText);
		fclose(fpText);
		fprintf(stderr," ** Source fileSize is %ld\n",fsize);
	}
	return fsize;
}
...
fprintf(stderr,"\n argc = %d",argc);
	...
	if ((argc !=7) && (argc !=8))  {
		...
		fprintf(stderr,"Use:  %s  <in file>  <out file> <numberOfPhrases> <min phrase len>< max phrase Lenght> <1/0 onlywords ?> [<stopwords file>]\n",argv[0]);
		return 0;
	}
