...
namespace quasi_succinct {
    ...
    struct and_query {
        ...
        {
			//uint32_t occs[100000000];  //fari
			
            if (terms.empty()) return 0;
            //remove_duplicate_terms(terms);

			typedef typename Index::document_enumerator enum_type;
			typedef std::pair<enum_type,int> and_type;
			
            std::vector<and_type> enums;
            enums.reserve(terms.size());

			int pos=0;
            for (auto term: terms) {
                enums.push_back(and_type(index[term],pos)); pos++;
            }

            // sort by increasing frequency
            std::sort(enums.begin(), enums.end(),
                      []( and_type const& lhs, and_type const& rhs) {
            ...
            candidate -=GAP1;
            size_t i = 1;
            while (candidate < (index.num_docs()-GAP1)) {
                for (; i < enums.size(); ++i) {
                    enums[i].first.next_geq(candidate + enums[i].second);
                    if (enums[i].first.docid() != (candidate + enums[i].second) ) {
                        candidate = enums[i].first.docid(); GAP1= enums[i].second;
                        candidate -=GAP1;
                        i = 0;
                        break;
                    }
				...
				//	occs[results] = candidate;
                    results += 1;
                    //candidate is a match!!
                    if (with_freqs) {
                        for (i = 0; i < enums.size(); ++i) {
                            do_not_optimize_away(enums[i].first.freq());
                        }
                    }
                    enums[0].first.next();
                    candidate = enums[0].first.docid();  GAP1= enums[0].second;
                    candidate -=GAP1;
                    i = 1;
                }
            }
