	...
	char *build_options, void **representation)
{
	...
	*representation = compressor;
	return compressor->build_representation(text, length, docBeginnings, numdocs, atoi(build_options));
}
...
int 
extract (void *representation, uint from, uint to, unsigned char **snippet, unsigned int *slen)
{
	RePairDocCompressor *compressor = (RePairDocCompressor*)representation;
	return compressor->extract(from, to, snippet, slen);
}
