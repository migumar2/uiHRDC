...
int save_il2 (void *ail, char *filebasename){
...
//	fprintf(stderr,"\n\t size of byteCodes = %d bytes",il->bcsSize);
	...
	fprintf(stderr,"\n\t size of: lenlist+clen+occs = %lu bytes; qmx-data = %lu bytes; byteCodes = %lu bytes",(ulong)sizetobits(ail), (ulong) il->cdataSize*sizeof(uint), (ulong)il->bcsSize);
