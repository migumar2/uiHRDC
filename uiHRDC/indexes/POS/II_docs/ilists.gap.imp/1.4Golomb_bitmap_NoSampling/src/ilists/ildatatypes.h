...
#define DATA_TYPES_IL_INCLUDED
	...
	typedef struct {	
		...
		size_t *occs;			// ptr to the begining of the occs of word i in zoneOccs.				
		...
		size_t bcsSize;		// number of used bits in gcs;
		size_t gcsSize;		// number of uints in gcs;
		...
		size_t bcpos; //  position inside bcs (botton level containing the compressed d-gaps).
		int val; // value up to that position
		uint j;
