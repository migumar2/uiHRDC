...
#define INTERSECT2_SVS_X 	('X')        //version fsearchUncompress
#define INTERSECT2_SKIPPING 	('K')    //version S[K]IPPING
...
void do_recover(char *filename);  //rebuilds the source posting lists

void do_intersect2svsUnc(uint numPhrases, uint REPETITIONS);
void do_intersect2svsExp(uint numPhrases, uint REPETITIONS);
void do_intersect2SKIPPING(uint numPhrases, uint REPETITIONS);
...
void endResultsRemoveZeros(  char *matlab, char* outfile, uint numIters) {
	FILE *f = fopen(outfile,"a");
	uint i,x;
	double y,sum;
	uint diffs =1;
...
/**/	fprintf(timeFile,"\n Searching!!: %s %s %s %s %s %s",argv[0],argv[1],argv[2],argv[3],argv[4], params);
	...
	writeSampleK_b(filename,sampleK);
	...
	switch (querytype){
		...
		case INTERSECT2_SVS_X:
			...
			do_intersect2svsExp((uint) atoi(argv[4]), (uint) atoi(argv[5]) );
			break;



		case INTERSECT2_SKIPPING:
			if (argc < 6) {
				print_search_usage(argv[0]);	
				exit (1);
			}
			if (argc > 6) 
				if (*argv[6] == VERBOSE){
						Verbose = 1;
						fprintf(stderr,"%c", INTERSECT2_SKIPPING);

				}
			do_intersect2SKIPPING((uint) atoi(argv[4]), (uint) atoi(argv[5]) );
			break;
...
void do_intersect2svsExp(uint numPhrases, uint REPETITIONS)
{
	...
	{ uint i;
		...
		for (iters=0;iters<REPETITIONS;iters++) {
			...
			//error = intersect_2_il (Index, patternSets[currPhrase][0], patternSets[currPhrase][1],
			//                               &numocc, &occs);
			g->svs2Exp ( patternSets[currPhrase][0], patternSets[currPhrase][1],  &numocc, &occs);
		}
...
/********************************************************/
void
do_intersect2SKIPPING(uint numPhrases, uint REPETITIONS)
{
	uint iters,currPhrase;
	...
	{	uint i;
		...
		for (iters=0;iters<REPETITIONS;iters++) {
			...
			g->int2Skipping ( patternSets[currPhrase][0], patternSets[currPhrase][1],  &numocc, &occs);
			
			{
				uint numocc2, *occs2;
				
				 		// ** check 					
					g->int2SkippingSeq ( patternSets[currPhrase][0], patternSets[currPhrase][1],  &numocc2, &occs2);
					{ int i;
					  if (numocc != numocc2) {
						printf("\n searches failed somewhere: numocc = %u, numocc2 = %u",numocc,numocc2);		exit(0);										  
					  }
					  else {
						int i;
						for (i=0; i< numocc ; i++) {
							if (occs[i] != occs2[i]) break;
						}
						if (i != numocc) {
							printf("\n searches failed (skipping-Seq-Search): occ[%u] = %u, occ2[%u] = %u",i,occs[i],i,occs2[i]);;exit(0);
						}						
						else
							printf("\n searches (skipping-Seq-Search) test finished OK ");
					...
					g->int2SkippingExp ( patternSets[currPhrase][0], patternSets[currPhrase][1],  &numocc2, &occs2);
					{ int i;
					  if (numocc != numocc2) {
						printf("\n searches failed somewhere: numocc = %u, numocc2 = %u",numocc,numocc2);	exit(0);											  
					  }
						...
						if (i != numocc)  {
							printf("\n searches failed (skipping-Exp-Search): occ[%u] = %u, occ2[%u] = %u",i,occs[i],i,occs2[i]);exit(0);
						}
						else
							printf("\n searches (skipping-Exp-Search) test finished OK ");
				  ...
				  /*
											...
											g->svs2Exp ( patternSets[currPhrase][0], patternSets[currPhrase][1],  &numocc2, &occs2);
											{ int i;
											  if (numocc != numocc2) {
												printf("\n searches failed somewhere: numocc = %u, numocc2 = %u",numocc,numocc2);	exit(0);											  
											  }
												...
												if (i != numocc)  {
													printf("\n searches failed:  svs2Exp: occ[%u] = %u, occ2[%u] = %u",i,occs[i],i,occs2[i]);exit(0);
												}
												else
													printf("\n searches (svs2-exp) test finished OK \n");
...
//		for (uint i=0;i<numocc; i++) {fprintf(stderr,"\n occ[%3d] ==> %d",i,occs[i]-1);}
	...
	uint multiply = 1; //we want times in milliseconds
	
	fprintf (stderr, "\nTotal Num occs found = %u\n", tot_numocc);
	fprintf (stderr, "Int2 time = %.8f msecs\n", tot_time*multiply);
	fprintf (stderr, "Int2_time/numOfPatterns = %.8f msecs/patt\n\n",
		 (tot_time * multiply) / numOfPatterns);
	fprintf (stderr, "(Load_time+Int2_time)/numOfPatterns = %.8f msecs/patt\n\n",
		 ((Load_time+tot_time) * multiply) / numOfPatterns);	

	fprintf (stderr, "Int2_time/occ = %.8f msecs per post \n\n",
		 (tot_time * multiply) / tot_numocc);   
		 
/**/	fprintf(timeFile,"\n\t occs = %9u :: time = %.8f msecs ",tot_numocc,tot_time*multiply);
/**/	fprintf(timeFile,"\n\t    avg time per pattern = %.8f msecs ",(tot_time * multiply) / numOfPatterns);
/**/	fprintf(timeFile,"\n\t      avg time per occur = %.8f msecs \n",(tot_time * multiply) / tot_numocc);
/**/	fprintf(timeFile,"\n\t      index size = %.4f Mbytes \n",Index_size/1024.0);  //in Megabytes
...
double
getTime (void)   // OJO!!! EN MILISEGUNDOS !!!
{
	double usertime, systime;
	struct rusage usage;
...
void print_search_usage(char * progname) {	
	...
	fprintf(stderr, "Usage:  %s <index> <sampleK>  <type> <numPatts> <repetitions> [V] [search_opts] < patt_file\n", progname);
	fprintf(stderr, "\n\t<type>   denotes the type of queries:\n");
	fprintf(stderr, "\t         %c <numListsToExtract><iterations per list>\n", RECOVER);
	fprintf(stderr, "\t             recovers the source postings - file\n");
	fprintf(stderr, "\t             Usage:  %s <index> <sampleK=1> R 0 0  [V] [search_opts] < patt_file\n", progname);
	...
	fprintf(stderr, "\t         %c <numFrases><iterations per frase>\n", INTERSECT2_SKIPPING);
	fprintf(stderr, "\t             Intersection of two lists (Using skipping)\n");
