...

	{
		...
		poss = occs[second]; //poss in zoneoccs	
       	...
       	for (k=ii; k<j; k++){ //for all elements in the second list...
    		...
    		uint nextSample = (k-ii)/sampleP;                            //non estaba antes... así que val podía valer calquera cousa 
			val = GetField(zoneOccs, bits_zoneOccs, poss + nextSample);  //non estaba antes... así que val podía valer calquera cousa
			

			if ( ((int) val - GAP2) < ((int)result[fi]-GAP1 +GAP)) {			
				/*exp search*/
			
//				uint nextSample = (k-ii)/sampleP;                        //esto estaba antes
				uint prevk=k, probe = 1;
//				val = GetField(zoneOccs, bits_zoneOccs, poss + nextSample);  //esto estaba antes
				
				while ( ((int) val -GAP2) < ((int)result[fi] - GAP1 +GAP) ) {
					currVal = val; 
					prevk= ii+(nextSample+1)*sampleP;
					nextSample += probe;
					k = ii+(nextSample)*sampleP; //incremento k s�lo si val < result[fi]
					if (k >= j) break;
					...
					
				{
					...
					while (r > l+1) {
						...
						if ( ((int)val -GAP2) < ( (int)result[fi] -GAP1 +GAP) ) l = m;
						else r = m;					
					}
				...
				/**peque�a optimizaci�n ???  evita resto bucle si coinciden **/
				/**fari 2008.12.26 **/
				if ( ((int)val-GAP2) == ((int)result[fi] - GAP1 +GAP)) {
					k++;
					currVal = val;
...
/****************************************************/	
...

	{
		...
		// 	uint prevkk = k;  // @TEST SAMPLING IS USED
			...
			while ( ((int)val -GAP2) < ( (int)result[fi] -GAP1 +GAP)) {
				currVal = val; k = ii+(++nextSample)*sampleP; //incremento k s�lo si val < result[fi]
				if (k >= j) break;
				val = GetField(zoneOccs, bits_zoneOccs, poss + nextSample);	
			}
	...
	if (numbytecodes == 0) { //intersecting only bitmaps --> mergeNbitmap
		...
		for (i=1; i<numbytecodes; i++){ // the "nids-1" remaining posting-lists.
...

			{
				...
				uint poss,val;
//				poss = occs[v]; //poss in zoneoccs	        //@@Fari 2015.03.27
				uint second = v+1;        					//@@Fari 2015.03.27
				poss = occs[second]; //poss in zoneoccs	    //@@Fari 2015.03.27
