...
int RepairPost::merge2 (uint id1, uint id2, uint *noccs, uint **occs){
	...
	uint *salida;
	//salida =  (uint *) malloc (sizeof(uint) * n1);
	salida = firstlist;
	
	while (1) {
		while ((i<n1) && ((firstlist[i]-GAP1) < (secondlist[j]-GAP2))) i++;
		if (i>=n1) break;

		while ((j<n2) && ((firstlist[i]-GAP1) > (secondlist[j]-GAP2) )) j++;
		if (j>=n2) break;			

		while ( (firstlist[i]-GAP1) == (secondlist[j]-GAP2) )  {
			salida[matches++] = firstlist[i]-GAP1;
			i++;j++;
			if ((i>=n1) || (j>=n2)) break; 
		}				
	}
...
//	}
	...
	//		printf("\n SORTED: pat %u: lenlist = %u, id = %u",i,idss[i].key, idss[i].value);
	//	printf("\n");
		...
		for (i=1; i<numbytecodes; i++){ // the "nids-1" remaining posting-lists.
			...
			{	register uint z,t;
				...
				while (1) {  //merges the two uncompressed lists.
					while ((z<n) && ( ((int)result[z]-GAP1) < ((int)secondlist[t]-GAP2) )) z++;
					if (z>=n) break;

					while ((t<n2) && ( ((int)result[z]-GAP1) > ((int)secondlist[t]-GAP2) )) t++;
					if (t>=n2) break;			

					while ( (result[z]-GAP1) == (secondlist[t]-GAP2) )  {
						result[matches++] = result[z]-GAP1;
						z++;t++;
						if ((z>=n) || (t>=n2)) break; 
					}				
				}   
