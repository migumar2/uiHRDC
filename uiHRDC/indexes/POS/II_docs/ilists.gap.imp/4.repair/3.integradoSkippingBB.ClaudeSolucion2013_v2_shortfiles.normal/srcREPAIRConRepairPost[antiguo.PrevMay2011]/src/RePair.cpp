...
void RePair::fillHeap() {
  ...
  hpos = new HashTablePos((uint*)data,m,(int*)(data+m+1),hashsize);   //** tabla hash es inicializada <posIni,contador>
	...
	for(ulong i=0;i<hashsize;i++)         //** INSERCIÓN EN EL HEAP ==> ya calcula los K más frecuentes.
...
ulong RePair::replacePairs() {
  ...
  ulong countR = 0;
	...
	for(ulong i=1;i<heap->inserted;i++)   //** cuenta el número de pares con freq > 1
...
//	  if ((100.*m/n)<60.0) break;	  
    ...
    ulong sorted=0;                           //** Normalmente no entra aquí !!! **/
