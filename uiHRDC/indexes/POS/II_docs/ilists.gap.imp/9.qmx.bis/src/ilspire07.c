...
int save_il (void *ail, char *filebasename){
	...
	int file;
	char *filename;	
	filename = (char *)malloc(sizeof(char)*(strlen(basename)+30));
	...
	
	{
		...
		ssize_t tranferred = fwrite(il->cdata, sizeof(uint),il->cdataSize, f);
		printf("\n QMX_data written = %lu uints",(ulong)tranferred);
		fclose(f);
	}
...
int load_il (char *filebasename, void **ail) {
	...
	{		
		...
		read(file, &(il->cdataSize), sizeof(uint));	
				printf("\n ####cdataSize VALE = %u bytes",	il->cdataSize);	fflush(stdout);fflush(stderr);
		...
		il->cdata[il->cdataSize -1] = 0000;
		//ssize_t leidos = read(file, il->cdata, il->cdataSize * sizeof(uint));					
		//				printf("\n ####cdata read bytes VALE = %lu bytes",	(ulong)leidos);	fflush(stdout);fflush(stderr);
	...
	
	{
		...
		ssize_t tranferred = fread(il->cdata, sizeof(uint),il->cdataSize, f);
		printf("\n QMX_data read = %lu uints",(ulong)tranferred);
		fclose(f);
	}	
...
//	*size += (il->nlists + 1)* sizeof(uint);  //il->bcsptr 
	...
	*size += sizetobits(ail);
	...
	sizeUint += il->nlists * sizeof(uint) ;   //clen vect
	sizeUint += (il->nlists+1) * sizeof(uint) ;   //occs vect

	*size += il->bcsSize ;
	*size += il->cdataSize*sizeof(uint) ;
	*size -= il->padUints * sizeof(uint);

//	fprintf(stderr,"\n\t size of: lenlist+occs = %d bytes Vs %d bytes (old)",sizetobits(ail),sizeUint);
//	fprintf(stderr,"\n\t size of byteCodes = %d bytes",il->bcsSize);

	fprintf(stderr,"\n\t size of: lenlist+clen+occs = %lu bytes; pfd-data = %lu bytes; byteCodes = %lu bytes",(ulong)sizetobits(ail), (ulong) il->cdataSize*sizeof(uint), (ulong)il->bcsSize);
	fprintf(stderr,"\n\t\t padding bytes = %lu bytes (skipped from final size)",il->padUints * sizeof(uint));
...
int extractList_il (void *ail, uint id, uint **list, uint *len) {
	...
	uint *alist = (uint *) malloc (sizeof(uint) * (lenlist + QMX_BS2));	
	register uint i;
	...
	else*/
	{
		...
		//uint *C = (uint *) malloc (100000000);
		//for (i=0;i<clen;i++)
		//	C[i] = cdata[i];
		//C[i++]=0;C[i++]=0;C[i++]=0;C[i++]=0;C[i++]=0;

		//s9_Decompression(cdata, alist, (int) lenlist);
		QMX_Decompression(cdata, (int) clen, alist, lenlist);
		//QMX_Decompression(C, (int) clen, alist, lenlist);
		//free(C);
		alist[0] -=DOCid_ADD;		
		for (i=1; i<lenlist;i++) {		
	...
	*len =i;		
	...
	/* extract a list and adds GAP to all the postings */
int extractList_gap_il (void *ail, uint id, uint **list, uint *len, uint GAP) {
	...
	*len =i;		
    ...
    /* Extracts the id-th list, having id \in [0..maxId-1] 
       Assumes list has enough space previously allocated ((lenlist + QMX_BS2) uints) */
...
int extractListNoMalloc_il (void *ail, uint id, uint *list, uint *len){
	...
	else */ 
	{
		...
		int clen = il->clen[id];
		//s9_Decompression(cdata, alist, (int) lenlist);
		QMX_Decompression(cdata, clen, list, lenlist);
		list[0] -=DOCid_ADD;		
		for (i=1; i<lenlist;i++) {		
...
int merge2 (void *ail, uint id1, uint id2, uint *noccs, uint **occs){
	...
	*/	
	...
	return merge_2_uncompressFstSIMD(ail,id1,id2,noccs,occs);
}
...
int merge_2_uncompressFstSIMD (void *ail, uint id1, uint id2,  uint *noccs, uint **occs){
	uint GAP1=1;
	uint GAP2=0;
    ...
    *occs = intersect_aux_simd(arr1,len1,arr2,len2,noccs);
    ...
    //RETURNS occurrences of the 1st pos of the phrase
    uint *res = *occs;
...
//	}
    ...
    *noccs = auxLen;
    ...
    //RETURNS occurrence of the 1st pos of the phrase
    register uint i, n = *noccs;
    uint *res=*occs;
    GAP=nids -1;
    for (i=0;i<n;i++) res[i] -=GAP;  
