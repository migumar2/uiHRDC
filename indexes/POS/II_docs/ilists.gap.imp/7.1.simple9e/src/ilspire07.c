...
int extractList_gap_il (void *ail, uint id, uint **list, uint *len, uint GAP) {
	...
	s9_Decompression(cdata, (uint *) alist, (int) lenlist);
	
	alist[0] -= DOCid_ADD;	//ADDS "GAP" to the postings.
	alist[0] +=  GAP;	//ADDS "GAP" to the postings.
	for (i=1; i<lenlist;i++) {		
		alist[i]=alist[i]+alist[i-1];//-DOCid_ADD;			
	}		
...
int merge_2_unc_s9 (void *ail, uint id1, uint id2,  uint *noccs, uint **occs){
	...
	uint len1;
    uint* arr1;
    extractList_gap_il(ail, id1, &arr1, &len1,+1);
    uint len2;
    //uint* arr2;
    //extractList_il(ail, id2, &arr2, &len2);
    //*occs = intersect_aux(arr1,len1,arr2,len2,noccs);
    ...
    s9_DecompressionAndIntersect_last2(cdata,(int) len2, ptr, ptrend, noccs,0,+1);
    //free(arr2);
...
int merge_N_unc_s9 (void *ail, uint *ids, uint nids, uint *noccs, uint **occs){
    ...
    for(unsigned int i=1;(i<nids-1)&&(auxLen);i++){
		...
		s9_DecompressionAndIntersect(cdata,(int) len, ptr, ptrend, &auxLen, GAP);
    }
    ...
    if (auxLen)	{
		...
		uint *ptr=auxArr;
		uint *ptrend = auxArr +auxLen;
        s9_DecompressionAndIntersect_last2(cdata,(int) len, ptr, ptrend, &auxLen, GAP, i);
    }      
 ...
 */
void s9_DecompressionAndIntersect(unsigned int* input, int size, uint *ptri, uint *ptrend, uint *matches, uint GAP) {
 ...
 * mark the new size of ptri[]
 */
void s9_DecompressionAndIntersect_last2(unsigned int* input, int size, uint *ptri, uint *ptrend, uint *matches, uint GAP, uint GAP_RESULT) {
	unsigned int output[28];	
	unsigned int *_p = output;
	int num;
	unsigned* tmp = input;
