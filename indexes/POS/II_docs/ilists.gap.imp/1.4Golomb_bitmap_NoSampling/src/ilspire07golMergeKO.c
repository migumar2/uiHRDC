	...
	*ail = il;
	...
	{	/** 4 ** creates a temporal list of occurrences of each word (block-oriented).
		...
		il->occs = (uint *) malloc (sizeof(uint) * il->nlists);
			...
			*/
			uint bcssize=0;
			{
				...
				fprintf(stderr,"\n Size of gcs Vector should be  %d bits (%d bytes)",bcssize, (bcssize +W-1)/W*sizeof(uint));
		...
		/** 5.c ** encodes gaps into gcs && set pointers into zoneoccurssTmp */
		{
			...
			fprintf(stderr,"\n Gaps were encoded with bc ==> %d bits (%d bytes)",bcpos, (bcpos+W-1)/W *sizeof(uint));			
		}		
...
//	*size += (il->nlists + 1)* sizeof(uint);  //il->bcsptr 
	...
	*size += sizetobits(ail);
	...
	fprintf(stderr,"\n size of compressed sequence = %d bits (%d bytes) \n",il->bcsSize, il->gcsSize*sizeof(uint));
