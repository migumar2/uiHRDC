...
#define MAXUINT  ((uint)1024*1024*2047)
...
{	ulong value; \
	value = (((ulong)maxvalue) * ((ulong)B) / ((ulong)len)); \
	CEILLOG_2(value,k); \
	val = 1<<k; \
}
*/
...
#define SAMPLEPERIOD(val, k, B, len, maxvalue) \
{	ulong valuel; \
	ulong kl,Bl,lenl,maxvaluel,ONE_l;\
	kl=k; Bl=B; lenl=len;maxvaluel=maxvalue;ONE_l=1;\
	valuel = ((maxvaluel * Bl) / lenl); \
	CEILLOG_2(valuel,kl); \
	val = ONE_l<<kl; \
	k=kl;\
}
...
#define NUMBUCKETS(val, maxval, k) \
{ ulong v_l,m_l,kk_l,ONE_l;\
  v_l=val;m_l=maxval;kk_l=k; ONE_l=1;\
 val = (m_l + (ONE_l<<kk_l) -1) /(ONE_l<<kk_l);\
}
