...
#include "RepairPost.h"
	...
	RepairPost::RepairPost(char * file) {
		...
		maxPostValue=computeMaxPostValue()+1;
		printf("\n ################################***************maxPostValue is %u ",maxPostValue);
		printf("\n ################################***************edges is %u\n ",n);
		assert(n==maxPostValue);
		createAux(file);	///2015!		
	}	

uint RepairPost::computeMaxPostValue() {
	printf("\n call to computeMaxPostValue()"); fflush(stderr);fflush(stdout);
	uint nl; uint *l;
	uint maxPost=0;		
	...
	for (i=1;i<=nodes;i++) {	
		l = adj_il(i,&nl);
		if (maxPost < l[nl-1]) maxPost= l[nl-1];
		free(l);
	}		
	return maxPost;
}
...
void RepairPost::createAux(char * file) {		  //DONE BY GONZALO !!! 
	...
	//load_bitmap(file); 
	printf("\n call to createAux()  [creates sampling structures]"); fflush(stderr);fflush(stdout);
	...
	{		
		...
		for (i=1;i<=nodes;i++) {	
			if (!shouldUseBitmap(i)) {  /** spire **/		
				SAMPLEPERIOD(sampleP,k,  B,lenPost[i], maxPostValue);
				NUMBUCKETS(n,  maxPostValue,k);
				numBuckets += n;
				totalbuckets +=n;
				numBucketsPerID[i]=n;
			}
	...
	uint maxAbsValue=0;
	{
		...
		for (id =1; id <=nodes; id++) {
			if (!shouldUseBitmap(id)) {  /** spire **/
				...
				uint sampleP,k;
				occs[id]=currPos;
				SAMPLEPERIOD(sampleP,k,  B,lenPost[id],maxPostValue);
		...
		bits_zoneOccsAbs = bits(maxPostValue);
		...
		fprintf(stderr,"\n maxPostValue = %d, bits_zoneOccsAbs = %d.", maxPostValue,bits_zoneOccsAbs);
...
//printf("\n int2SkippingLookup \n");fflush(stdout);fflush(stderr);
	...
	if (shouldUseBitmap(id1) && shouldUseBitmap(id2)) {
		...
		ids[0]=id1; ids[1]=id2;
		GAPS[0]=0; GAPS[1] = 1;
		int err =  mergeNBitmap (&ids[0], nids, GAPS, noccs, occcs);
		return err;
	}
	...
	SAMPLEPERIOD(sampleP,k,  B,lenPost[second], maxPostValue);
	NUMBUCKETS  (n, maxPostValue,k);
	PTRBITS     (ptrbits, ptrs,plen, second);  //lenbits of sample-pointers for current id.

	//printf("\n sampleP=%u ,k=%u ,B=%u ,lenPost[second]=%u, maxPostValue=%u ",sampleP,k, B, lenPost[second], maxPostValue);
	//printf("\n n=%u",n);
	//printf("\n ptrbits=%u, ptrs=%u, plen=%u",ptrbits,ptrs,plen);
	...
	{	register uint symb2nd, val2nd;
		...
		register uint S,E;
    	while (fi < m) {  // processes all the elements of the 1st list
    		E=0;
    		h = (M[fi] +(GAP2-GAP1)  +GAP )>>k;  //d div 2^k  == bucket in 2nd list for value "M[fi] +(GAP2-GAP1) +GAP"
    		currPtr = ZONEOCCSPTR(second, h, n, bits_zoneOccsAbs, ptrbits);
    		R=fi+1;
    		
    		if ((R<m)) {
    			nextPtr = ZONEOCCSPTR(second, ((M[R]+(GAP2-GAP1)+GAP)>>k), n, bits_zoneOccsAbs, ptrbits);
				...
				{ //checks the case were the last phrase expands into next bucket.
				  //moves R forward and sets E to 1 (expand 1 extra-phrase after this bucket)
					if ((nextPtr < y) && (R<m)) {
						uint val= ZONEOCCSABS(second, ((M[R]+(GAP2-GAP1)+GAP)>>k), bits_zoneOccsAbs);
												;						
						...
						if ( val >= (((M[R]+(GAP2-GAP1)+GAP)>>k)*sampleP)) { 
							//this symbol should be also expanded... so move R forward.
							E=1;
							while ((R<m) && ((M[R]+(GAP2-GAP1)+GAP) <= (val))) R++;
						}
					}
				}								
			}
    		...
    		if (R <m)  //setting E (expanding symbols from S to E-1
    			E += x+ ZONEOCCSPTR(second, ( (M[R]+(GAP2-GAP1)+GAP)>>k), n, bits_zoneOccsAbs, ptrbits);
    		else E=y-1; // *  FARI 22.05.2009 **

    		currVal =  ZONEOCCSABS(second, h, bits_zoneOccsAbs);	

			//intersects M[fi ..fr] with symbols[S..E)								
			while ((fi<=fr) && (S<=E) && ( currVal  < (M[fr]+(GAP2-GAP1) +GAP) )) { // *  FARI 22.05.2009 **  //expanding 1-frase at a time.
    			...
    			if (symb2nd <= max_value) { // a terminal value --> no expansion is needed.	    			
					...
					while ( (fi <= fr) && ((M[fi]+(GAP2-GAP1) +GAP) < currVal) ) fi++;
					if (fi>fr) break;
					if ((M[fi]+(GAP2-GAP1)+GAP) ==  currVal ) {
    				...
    				if ( val2nd == (M[fi]+(GAP2-GAP1) +GAP)) {
    					result[matches++]=val2nd-GAP2-GAP; 
    					fi++;
					}
 					else if (val2nd > (M[fi]+(GAP2-GAP1) +GAP)) {  //processing is needed 
 						fl=fi+1;
 						while ((fl<=fr) && ((M[fl]+(GAP2-GAP1)+GAP) <= val2nd)) fl++; //phrase expands through more buckets
 						fl--;
...
/*
int RepairPost::int2SkippingLookupxxx(uint id1, uint id2, uint *noccs, uint **occcs){ 
...
/*
int RepairPost::int2SkippingLookupVV(uint id1, uint id2, uint *noccs, uint **occcs){ 
...
//printf("\n int2SkippingLookup \n");fflush(stdout);fflush(stderr);
	...
	{	register uint symb2nd, val2nd;
    	...
    	while (fi < m) {  // processes all the elements of the 1st list
    		E=0;
    		h = (M[fi]-GAP1+GAP )>>k;  //d div 2^k  == bucket in 2nd list for value "M[fi]-GAP1+GAP"
    		currPtr = ZONEOCCSPTR(second, h, n, bits_zoneOccsAbs, ptrbits);
    		R=fi+1;
    		
    		if ((R<m)) {
    			nextPtr = ZONEOCCSPTR(second, ((M[R]-GAP1+GAP)>>k), n, bits_zoneOccsAbs, ptrbits);
    			while ((R<m) && (nextPtr == currPtr)) {
					R++;
					if ((R<m)) {
						nextPtr=ZONEOCCSPTR(second, ((M[R]-GAP1+GAP)>>k), n, bits_zoneOccsAbs, ptrbits);						
				...
				{ //checks the case were the last phrase expands into next bucket.
				  //moves R forward and sets E to 1 (expand 1 extra-phrase after this bucket)
					if ((nextPtr < y) && (R<m)) {
						uint val= ZONEOCCSABS(second, ((M[R]-GAP1+GAP)>>k), bits_zoneOccsAbs);
												;						
						//reads skipping info for the 1st symbol of next bucket
						symb2nd = GetField(symbols,nbits,(nextPtr)); 						
						if (symb2nd <= max_value) 
						...
						if (((int)val-GAP2) >= (( ((int)M[R]-GAP1+GAP)>>k)*sampleP)) { 
							//this symbol should be also expanded... so move R forward.
							E=1;
							while ((R<m) && (((int)M[R]-GAP1+GAP) <= ((int)val -GAP2))) R++;
						}
					}
				}								
			}
    		...
    		if (R <m)  //setting E (expanding symbols from S to E-1
    			E += x+ ZONEOCCSPTR(second, ( (M[R]-GAP1+GAP)>>k), n, bits_zoneOccsAbs, ptrbits);
    		else E=y-1; // *  FARI 22.05.2009 **

    		currVal =  ZONEOCCSABS(second, h, bits_zoneOccsAbs);	

			//intersects M[fi ..fr] with symbols[S..E)								
			while ((fi<=fr) && (S<=E) && ( ((int)currVal-GAP2 ) < ((int)M[fr]-GAP1 +GAP) )) { // *  FARI 22.05.2009 **  //expanding 1-frase at a time.
	
				symb2nd = GetField(symbols,nbits,S);  // the k-th element of the 2nd list
    			
    			if (symb2nd <= max_value) { // a terminal value --> no expansion is needed.	    			
					currVal += symb2nd;  //the actual value	
					while ( (fi <= fr) && (((int)M[fi]-GAP1 +GAP) < ((int)currVal-GAP2)) ) fi++;
					if (fi>fr) break;
					if (((int)M[fi]-GAP1+GAP) == ((int) currVal - GAP2)) {
						result[matches++] = currVal-GAP2 -GAP;
						fi++;
					}										
				}
    				...
    				val2nd = currVal + GetField(csymb_n, bits_sn, (symb2nd-max_value-1)); //uses skipping-info.    			 
    				if ( ((int)val2nd-GAP2) == ((int)M[fi]-GAP1 +GAP)) {
    					result[matches++]=val2nd-GAP2-GAP; 
    					fi++;
					}
 					else if (((int)val2nd-GAP2) > ((int)M[fi]-GAP1 +GAP)) {  //processing is needed 
 						fl=fi+1;
 						while ((fl<=fr) && (((int)M[fl]-GAP1+GAP) <= ((int)val2nd-GAP2))) fl++; //phrase expands through more buckets
 						fl--;
 						expandAndSearchSkipping (symb2nd); //currValue, fi, fl, result.
 						assert (fi== (fl+1)); //fi=fl+1;
					}					
...
*/	
	...
	while (1) {
		while ((i<n1) && (((int)firstlist[i]-GAP1) < ((int)secondlist [j]-GAP2) )) i++;
		if (i>=n1) break;

		while ((j<n2) && ( ((int)firstlist[i]-GAP1) > ((int)secondlist [j]-GAP2) )) j++;
		if (j>=n2) break;			

		while (( (int)firstlist[i]-GAP1) == ((int)secondlist [j]-GAP2))  {
...
char * RepairPost::showFirst(uint id, uint n, uint n1) {
...
//	fprintf(stderr,"\n intNskipping:: numbitmaps= %u, numbytecodes = %u", numbitmaps,numbytecodes);
	...
	if (numbytecodes == 0) { //intersecting only bitmaps --> mergeNbitmap
		int err =  mergeNBitmap ( idsBitmaps, numbitmaps, gapsBitmaps, noccs, occcs);		
		free(idss);
		free(idsBitmaps);
		free(gapsBitmaps);
		return err;
