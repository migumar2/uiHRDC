...
/* Reads the file containing uints. */
int read_constants(char *filename, uint *sourcelen, uint *maxLenPosting, uint *maxPostingValue, uint *divBitmap) {
...
printf("\n\n opening file %s",filename);
  ...
  fread(sourcelen,sizeof(int),1,infile);
  fread(maxLenPosting,sizeof(int),1,infile);
  fread(maxPostingValue,sizeof(int),1,infile);  /******************* spire ***********/
  fread(divBitmap,sizeof(int),1,infile);     /******************* spire ***********/
  
  printf("\n ## Constants:##: sourcelen=%d, maxLenPost = %d, MaxPostValue = %d, divBitmap = %d\n",
...
int main(int argc, char ** argv) {
  if(argc<3) {
	...
	char * outfile = argv[2];
  	...
  	uint *source; uint source_len;  //source file containing original posting lists.
	...
	{ //loads the file with the constants.
		...
		uint sourcelen, maxlenPosting ,maxPostingValue1;
		...
		printf("\n constantsRead: nlist = %d, sourcelen = %d",nlists,sourcelen);
