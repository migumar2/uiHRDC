...
int build_il (uint *source, uint length, char *build_options, void **ail) {
	...
	*ail = il;
	...
	/** processing the parameters */
	{
		...
		sprintf(msg,"%s; replacesPerIter = %u; repairBreak=%2.3f %%; filename =%s\"",text1,replacesPerIter, repairBreak, filename);
		printf("\n Build parameters of method: %s", msg);
		
		sprintf(text1,"\"path2convert=%s; path2build_bitmap= %s ",path2convert,path2build_bitmap);
		sprintf(msg,"%s; path2build_index = %s; path2compress_dictionary=%s \"",text1,path2build_index, path2compress_dictionary);
	...
	{ //use external programs to repair compression of the lists + bitmaps.
	  //creating "filename.*" files.
		...
		if (strcmp(path2build_index,"")){
			sprintf(command,"%s %s %u %u %2.5f", path2build_index, filename, maxMemRepair, replacesPerIter, repairBreak );
		 ...
		 else {printf("\n Path to **build_index program** not provided: I cannot continue\n"); exit(0);}
		...
		for (j=0; j<num_parameters;j++) {
			...
			if ((strcmp(parameters[j], "int2") == 0 ) && (j < num_parameters-1) ) {
				if (strcmp(parameters[j+1], "svs") == 0 )      //just for testing.
					il->defs.defaultIntersect2 = &RepairPost::svs2Unc;
