...
{	
	...
	{	
	  ...
	  printf("\n\tLoaded Source Text into memory from %s", argv[1]);	  
	  ...
	  /** MIGUEL: en el build_options estoy pasando el sampling !!!! */
	  void *ct;
	  build_representation (text, textSize, docboundaries, ndocs, argv[4] , &ct);
	  ...
	  printf("\n\t Ratio = %2.4f%% \n", (double)compsize/textSize);			  
  
	  /** 5 ** Saves the compressed representation to disk **/
	  save_representation(ct, argv[3]);	
	  printf("\n The compressed representation is saved to disk: %s",argv[3]);
	  ...
	  for (i=0;i<textSize;i++) {
		if (text[i] != textDec[i]){
			printf("\n decompression failed in recovering the original text!!!");
			exit(-1);
		}
	  }
	  ...
	  /** 12 ** Writes out the decompressed textDec.*/
		extract_from_to_char (ct, 1,100, &document, &doclen);
	  printf("\n === Decompression of the bytes 1..100 ===");
	  printf("\n%s",document);
	  printf("\n =============== ***** ===================\n");
		free(document);
	  ...
	  /** 14 ** frees the ct structure that represents the source text in compressed form **/
	  free_representation(ct);	  
	...
	{
		printf( "\n");
		printf( "Usage: %s <input file text> <input file docbeginings> <output file> <""sample_ct=16384"">\n", argv[0]);
