	...
	{ //use external programs to repair compression of the lists + bitmaps.
	  //creating "filename.*" files.
		...
		if (strcmp(path2convert,"")){
			...
			if (length != fwrite(source, sizeof(uint),length,f)) {
				fprintf(stderr,"\n ilistimp: unable to write tmp_source_data (%lu bytes): to convert\n", length);
				exit(0);
			}
			fclose(f);			
		...
		if (strcmp(path2build_bitmap,"")){
			...
			if (length != fwrite(source, sizeof(uint),length,f)) {
				fprintf(stderr,"\n ilistimp: unable to write tmp_source_data (%lu bytes): to buildbitmap\n", length);
				exit(0);
			}			
			fclose(f);			
		...
		for (j=0; j<num_parameters;j++) {
			...
			if ((strcmp(parameters[j], "int2") == 0 ) && (j < num_parameters-1) ) {
				if (strcmp(parameters[j+1], "svs") == 0 )      //just for testing.
					...
					il->defs.defaultIntersect2 = &RepairPost::merge2;	    
				j++;
			}
			...
			//setting the default function to Intersect-N-lists		
				...
				if (strcmp(parameters[j+1], "skip") == 0 ) 
					...
					il->defs.defaultIntersectN = &RepairPost::mergeN;			
				j++;
			}
		}
