...
int main(int argc, char ** argv) {	  
  ...
  size_t mem = atoi(argv[2]);
  uint K = atoi(argv[3]);

  uint nodes; long edges;
  fread(&nodes,sizeof(uint),1,fp);
  printf("\n\n\n ***** nodos = %u ",nodes); //** reads the number of nodes
  fread(&edges,sizeof(long),1,fp);          //** reads the number of edges
  printf("\n\n\n *****ejes = %ld\n ",edges);
fflush(stdout);
  ...
  mem = nodes + edges +1;   // *PUESTO QUE NO HAREMOS REPAIR, sino que se delega en el IRepair de Gonzalo, no se necesita buffer extra! 
	...
	size_t * offsets = new size_t[nodes+1];        //**  ej real. offsets= [0][13][17][19][19][20][20][23][28][33]

  int min = 0; uint nodes_read=0;
  bool fst = false; int last=0;
  for(ulong i=0;i<nodes+edges;i++) {
    int k;                             
    fread(&k,sizeof(uint),1,fp);   // ** reads either [nodeId <0] or [edgeId>0].
    if(k<0) {   //** a node
      //bitset(bitmap,i);
      offsets[-k-1]=i+k+1;
      if(k<min) min=k;
...
///** FARI */
  ...
  {  sprintf(fname,"%s.ptr.ulongs",argv[1]);   ////// Este fichero no se usa, pero es m�s facil ver el .ptr.ints que el .ptr ;)
	FILE * f_out = fopen64(fname,"w");
	fwrite(&min,sizeof(int),1,f_out);
	fwrite(&nodes,sizeof(uint),1,f_out);
	fwrite(&edges,sizeof(long),1,f_out);  	
	fwrite(offsets,sizeof(size_t),nodes+1,f_out);
	fclose(f_out);
  }
 ...
 /* 
  ...
  uint offsets_len = (nodes+1)*plen/WW64+1;
  size_t * off = new size_t[offsets_len];   // ** offsets codificados como "k-bit intergers"
  off[offsets_len-1]=0000; //  evita  que proteste valgrind 
  for(uint i=0;i<=nodes;i++)
    setField64Fari(off,plen,i,offsets[i]);
  delete [] offsets;
  offsets=off;
  */
...
/*
  ...
  fwrite(&nodes,sizeof(uint),1,f_out);
  fwrite(&edges,sizeof(long),1,f_out);
  fwrite(offsets,sizeof(size_t),offsets_len,f_out);   //codificados con K-bits (plen bits de hecho)
  fclose(f_out);
*/
