...
uint BitRankW32Int::prev(uint start) {
      ...
      while (!val) { val = data[--i]; answer -= WW32; }
...
uint BitRankW32Int::next(uint k) {
        ...
        for (uint i=count/WW32+1;i<integers;i++) {
                aux2=data[i];
                if (aux2 > 0) {
                        if ((aux2&0xff) > 0) return i*WW32+select_tab[aux2&0xff]-1;
                        else if ((aux2&0xff00) > 0) return i*WW32+8+select_tab[(aux2>>8)&0xff]-1;
                        else if ((aux2&0xff0000) > 0) return i*WW32+16+select_tab[(aux2>>16)&0xff]-1;
                        else {return i*WW32+24+select_tab[(aux2>>24)&0xff]-1;}
                }
        }
        return n;
} 
