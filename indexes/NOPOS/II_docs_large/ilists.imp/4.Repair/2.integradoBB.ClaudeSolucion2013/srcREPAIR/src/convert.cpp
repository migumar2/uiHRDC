...
int read_file(char *filename, uint **sourceInts, ulong *length) {
  ...
  *length = ((size_t) ftell(infile))/(sizeof(uint));
  ...
  fflush(stdout);
  cout << " [convert] I have read " << (*length)*sizeof(uint) << " bytes from postings.post file" ;
  cout << " [convert] I have read " << (count)*sizeof(uint) << " bytes from postings.post file" ;
...
int main(int argc, char ** argv) {
  ...
  posts = inlength - buff[0] -2;    //posible overflow en fichero grande @@fari !!
  
  cout << "inlen " << inlength << " " ;
  
  fwrite(&posts,sizeof(long),1,fp); //number of posting values.  //not used in advance !!!
  ...
  ulong actualPosts = 0; //counts the number of values managed with re-pair. --> sequence for repair.
  ...
  for ( i=0; i<terms; i++) {
    ...
    for (int j=0; j<len;j++) {
      value = buff[sourcepos++];
      value +=1;  //** para que el valor de posting m�s bajo sea "1" y no "0"
  ...
  fwrite(&actualPosts,sizeof(long),1,fp); //number of posting values.
