...
int gonzalosRepair(int * text, uint nodes, ulong edges, char *fname, char **argv, char * path2irepair, double CORTE_REPAIR)
{  /// USES GONZALO'S irepair to create the C-sequence and the dictionary of rules.
		...
		//procesar C  --> para generar fichero .rp compatible
			...
			//bool RePair::saveRPFile(ulong nodes) {
				{
				  ...
				  uint mm= (uint) m;
					fwrite(&mm,sizeof(uint),1,fp);
					fwrite(&n,sizeof(ulong),1,fp);         //longitud de la secuencia de repair original (nodes + ejes), ver arriba.
				  ...
				  fseeko64(fp,3*sizeof(uint),SEEK_SET);          //** reemplaza el valor "m".
					fwrite(&writ,sizeof(uint),1,fp);
				  n -= (m-writ);                                  //** taman real del texto comprimido quitando "nodos"
					fwrite(&n,sizeof(ulong),1,fp);                //** reemplaza el valor "n".
					fclose(fp);
				}
			//}		
