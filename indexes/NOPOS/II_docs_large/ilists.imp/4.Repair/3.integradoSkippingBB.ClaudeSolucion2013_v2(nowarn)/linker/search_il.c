...
static ulong Index_size, Uncompressed_Index_size ;
...
void readPatternSetsIntersect (uint *numIdsPerPattern, uint *numOfPatterns, uint ***patternSets) {
	int rfs;
	rfs= fscanf(stdin,"##numIdsPerPattern: %9d\n",numIdsPerPattern);
	rfs= fscanf(stdin,"##numOfPatterns : %9d\n",numOfPatterns);
...
void readPatternSetsExtract (uint *numOfIDs, uint **IDs) {
	...
	*numOfIDs = numIdsPerPattern * numOfPatterns;
	...
	for (i=0; i< (*numOfIDs); i++) {rfs= fscanf(stdin,"%9d\n",&value); V[i]=value; }
	*IDs = V;
