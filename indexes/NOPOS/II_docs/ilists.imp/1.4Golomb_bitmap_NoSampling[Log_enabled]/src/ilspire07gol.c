...
int build_il (uint *source, uint length, char *build_options, void **ail) {
	...
	*ail = il;
	...
	{	/** 4 ** creates a temporal list of occurrences of each word (block-oriented).
		...
		printf("numOflists = %lu, maxPost = %lu, lenBitmapThreshold = %lu \n", 	
											(ulong)il->nlists, (ulong)maxPostValue, (ulong)il->lenBitmapThreshold);
...
inline int fsearchSeqMem (void *ail, uint id, uint value) {
	...
	while ((j<lenlist) && (val < value)) {
		lookupNext(gcs, &bcpos, &j, lenlist, &val, b_g, il->golombRiceTables, value);
	}
