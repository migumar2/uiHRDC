...
typedef struct{
    void* extractor;
    //void* doc_array;
    unsigned int* encodedLen;   //pointers to start of encoded v-byted list
    unsigned int bcs_encodedLen_n;  //len of the byte-coded posting lists (with are indexed by the lz-end idx
    
    //unsigned int* docLen;
    
    unsigned int *lenlist;   // num of elements of each posting list
    unsigned int lenlistbits; // bits per each element in lenlist[]
    unsigned int lenlistnints; //number of integers into which the "doc" values in "doclen" are encoded bit-wise with lenlistbits bits each
    unsigned int maxLength;
    
        
    unsigned int docs;   //number of words in the vocabulary
    
    unsigned int delta_sampling; /** fari 2013 **/
} lzi;
...
int merge2uncBCS (uint id1, uint n1, unsigned char *src1, uint id2, uint n2, unsigned char* src2, uint *noccs, uint **occs);
