...
LZEnd2::LZEnd2(char* filename):LZparser(filename){
    int temp;
    char filename_rev[2048];
    //std::cout<<"start"<<std::endl; 
    //sleep(10);
    std::cout<<" call to create reverseText (from LZEnd2::LZEnd2)\n**********************"<<std::endl;
    ...
    std::cout<<" to create SA (LZEnd2::call to getSA(--* text_len: " << this->text_len << " ** -)\n**********************"<<std::endl;
    ...
    std::cout<<"  LZEnd2::call to static_permutation_mrrr(---)  \n*********************"<<std::endl;
...
unsigned int LZEnd2::SAI(unsigned int pos){
    return this->pi->rev_pi(this->text_len- 1 - pos);
    //return this->sai[pos];
}
unsigned int LZEnd2::SA(unsigned int pos){
...
void LZEnd2::precomputeOcc(unsigned int** pc1, unsigned short int** pc2,unsigned int len){
   int b_size = 512;
   unsigned int i,j;
...
unsigned int* LZEnd2::precomputeOcc2(int bucket_size,unsigned int len){
   unsigned int i,j;
   unsigned int occ_temp[256];
...
unsigned int LZEnd2::Occ2(unsigned char c,int pos){
   int i;
   int bucket_pos;
   unsigned int occ;
   if(pos==0)return 0;
...
unsigned int LZEnd2::Occ(unsigned char c,int pos){
   int i;
...
void LZEnd2::get_rows(int* nmin,int* nmax,int first,int last,unsigned char c,unsigned int* C){
...
unsigned char LZEnd2::L(int pos){
    return this->La[pos];
    /*int t = this->SA(pos);
    if(t==0)return '\0';
...
unsigned int LZEnd2::add_bits(unsigned int pos, unsigned int len){
    unsigned int b=0;
