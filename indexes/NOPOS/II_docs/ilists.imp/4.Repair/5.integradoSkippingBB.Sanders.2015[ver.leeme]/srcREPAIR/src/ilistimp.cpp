	...
	parse_parameters_il(search_options,&num_parameters, &parameters, delimiters);
		...
		for (j=0; j<num_parameters;j++) {
			...
			if ((strcmp(parameters[j], "int2") == 0 ) && (j < num_parameters-1) ) {
				if (strcmp(parameters[j+1], "merge") == 0 )      //just for testing.
					il->defs.defaultIntersect2 = &RepairPost::merge2;
				else if (strcmp(parameters[j+1], "lookup") == 0 )
					il->defs.defaultIntersect2 = &RepairPost::int2SkippingLookup;	    
				else 
					il->defs.defaultIntersect2 = NULL;	    
				j++;
			}
			...
			//setting the default function to Intersect-N-lists		
				...
				if (strcmp(parameters[j+1], "lookup") == 0 ) 
					il->defs.defaultIntersectN = &RepairPost::intNSkippingLookup;			
