...
int getDepthSymbolFari(uint s, uint iniValue) {
  if(s>max_value) {
    return 1+(max ( getDepthSymbolFari(symbols_pair[  2*(s-max_value-1)],iniValue), 
                    getDepthSymbolFari(symbols_pair[1+2*(s-max_value-1)],iniValue)
                 )
           );
  }
  return iniValue+1;
...
int getMaxDepthFari() {
  int currMax=0;
  for(uint i=0;i<m;i++) {
    currMax= max (currMax, getDepthSymbolFari(symbols[i],0));
  }
return currMax;
}
/******************************************************************************/
...
void compress_pair_table() {
  ...
  cout << "MAX DEPTH OF A SYMBOL IS " << getMaxDepthFari() << " !!!\n**************************\n";
...
int main(int argc, char ** argv) {
  ...
  assert(symbols_pair!=NULL);
	...
	rf= fwrite(&n,sizeof(uint),1,cpsifp);
  uint wr = 0;
